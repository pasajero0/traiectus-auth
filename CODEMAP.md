...
| `packages/ui` | Shared components and design tokens, sign-in pages included | — |
...
| `sso-api` | `src/index.ts` → `src/server.ts` → `src/routes/` | 4000 |
| `*-web` | `src/app/` (routes) · `src/server/` (server-only logic) | 3000, 3001 |
| `*-api` | `src/index.ts` → `src/app.ts` → `src/routes/` → `src/domain/` | 3100, 3101 |
