...
async function accessToken(
  ...
  return new SignJWT({})
    ...
    .setAudience(claims.audience ?? 'harbor')
...
describe('GET /v1/projects', () => {
  it('refuses a request with no Authorization header', async () => {
    const response = await buildApp(env).request('/v1/projects')
  ...
  it.each([
    ...
    const response = await buildApp(env).request('/v1/projects', { headers: { authorization } })
  ...
  it('refuses a token minted for another product', async () => {
    const token = await accessToken({ audience: 'beacon' })
