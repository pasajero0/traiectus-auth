...
beforeEach(() => {
  ...
  process.env['TRAIECTUS_CLIENT_ID'] = 'beacon'
  ...
  process.env['BEACON_API_URL'] = 'http://localhost:3101'
})
...
describe('the Origin check', () => {
  ...
  it('refuses a request from another site', async () => {
    expect((await logout({ origin: 'https://not-beacon.example' })).status).toBe(403)
  })
})
...
describe('signing out', () => {
  it('clears the local cookie and sends the browser to the hub, session intact', async () => {
    ...
    expect(response.headers.get('set-cookie')).toContain('beacon_session=;')
  })
})
